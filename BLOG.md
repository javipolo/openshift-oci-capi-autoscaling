...
1. [**Oracle Cloud CLI (oci-cli)**](https://docs.oracle.com/en-us/iaas/Content/API/SDKDocs/cliinstall.htm) installed and configured with proper credentials.
...
8. **Oracle Cloud Account** Oracle cloud account with API access.

## Manual Component Installation

For now we have to install and configure all components manually.
All this process can be streamlined by creating an operator that manages all the components in this stack.
...
```
Then, we will set all needed values to import the running cluster. There are two different methods that can be used:
...
If your cluster is installed in a different way, or you're not sure, just find the OCID of each object and set the proper
variables
```bash
